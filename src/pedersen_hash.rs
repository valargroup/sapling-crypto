...
{
    // Buffer the full bit stream so we know the exact chunk count up front. This lets us index
    // precomputed blocks without ever reading past the end of the message: the trailing bits of
    // the final chunk are zero-padded (matching Sapling's definition), but chunks beyond the
    // message must never be added.
    let bits: Vec<bool> = personalization.get_bits().into_iter().chain(bits).collect();
